...
export function WorkflowCanvas({ initialWorkflow, onBack }: WorkflowCanvasProps): ReactElement {
  ...
  return <section className="canvas-shell">
    ...
    <aside className="node-palette"><strong>Add node</strong><small>Drag onto canvas or click</small>{paletteNodes.map((template) => <button key={template.name} draggable onDragStart={(event) => startNodeDrag(event, template.name, template.type, template.parameters)} onClick={() => addNode(template.name, template.type, template.parameters)}>{template.label}</button>)}</aside>
    ...
    <button className="test-input-toggle" type="button" aria-label="Open test input" title="Test input" onClick={() => setShowTestInput(true)}>⌘</button>
    ...
    {selectedNode!==undefined&&<button type="button" className="node-delete" onClick={() => { setWorkflow((current) => ({ ...current, nodes: current.nodes.filter((node) => node.id !== selectedNode.id), connections: current.connections.filter((connection) => connection.sourceNodeId !== selectedNode.id && connection.targetNodeId !== selectedNode.id) })); setSelectedNodeId(undefined); setShowNodeFile(false); }}>Delete node</button>}
